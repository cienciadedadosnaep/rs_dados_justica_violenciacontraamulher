...
library(readr)
dados_estrup <- read_csv("data/dados  - estrup.csv")
dados2 <- dados_estrup %>% select(ano,casos_estrup)
nomes <- names(dados2)
nomes[2] = 'Total'
##  Perguntas e titulos 
...
objeto_0 <- dados2 %>%
  ...
  select(ano,casos_estrup) %>% #filter(ano<2019) %>%
  #arrange(trimestre) %>%
...
data_serie <- paste('[',gsub(' ',',',
                             paste(paste(as.vector(objeto_0[[1]]$casos_estrup)),
                                   collapse = ' ')),']',sep = '')
