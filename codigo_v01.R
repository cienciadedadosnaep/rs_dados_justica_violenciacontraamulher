...
dados <- dados_viol_sex_bahia_raca_cor %>% select(ano,negras,nao_negras,ignorado)
names(dados) <- c("Ano","Negras","Não negras","Sem informação")
nomes <- names(dados)
##  Perguntas e titulos 
...
objeto_0 <- dados %>%
  ...
  select(Ano,Negras,`Não negras`,`Sem informação`) %>% #filter(ano<2019) %>%
  #arrange(trimestre) %>%
...
subtexto<-"Fonte: Rede de Observatórios da Segurança"
...
data_serie <- paste('[',gsub(' ',',',
                             paste(paste(as.vector(objeto_0[[1]]$Negras)),
                                   collapse = ' ')),']',sep = '')

data_serie2 <- paste('[',gsub(' ',',',
                              paste(paste(as.vector(objeto_0[[1]]$`Não negras`)),
                                    collapse = ' ')),']',sep = '')
...
texto<-paste('{"title":{"text":"',titulo,
             ...
             '"data":',data_axis,'},',
             '"yAxis":{"type":"value","axisLabel":{"formatter":"{value}"},"max":950},',
